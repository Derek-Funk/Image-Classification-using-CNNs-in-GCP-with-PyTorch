...
# other set up ---------------------------------------------------------------------------------------------------
device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu") #not working?
torch.manual_seed(0)
...
#######################################################################
for image in listOfTrainingImageFilenames[0:10000]:
...
class CNN(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(3, 8, (3,3))  # output (n_examples, 8, 48, 48)
        self.convnorm1 = nn.BatchNorm2d(8)
        self.pool1 = nn.MaxPool2d((2, 2))  # output (n_examples, 8, 24, 24)
        self.conv2 = nn.Conv2d(8, 32, (3,3))  # output (n_examples, 32, 22, 22)
    ...
    def forward(self, x):
        ...
        x = self.softmax(self.linear1(x.view(len(x), -1)))
        x = self.linear1_bn(x)
        x = self.drop(x)
        x = self.linear2(x)
        return x
...
for epoch in range(N_EPOCHS):
    ...
    print("Epoch {} | Train Loss {:.5f}, Train Acc {:.2f} - Test Loss {:.5f}, Test Acc {:.2f}".format(
        epoch, loss_train / BATCH_SIZE, acc(xTrainTensor, yTrainTensor), loss_test, acc(xTestTensor, yTestTensor)))

# save model -----------------------------------------------------------------------------------------------------------
modelFilename = 'model_derek_funk.pt'
torch.save(examModel.state_dict(), modelFilename)